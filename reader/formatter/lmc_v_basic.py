...
class lm_v_basic:
    ...
    def process(self, data, mode = "train"):
        ...
        for item in data:
            ...
            wt = item['lesion_ratio']
            feature = [item['Sex'],item['Age'],item['AFP'],item['GGT'],item['HBsAg'],item['ALP'],item['AST'],item['ALT'],item['ALB'],item['TBIL'],item['DBIL'],item['CEA']]
            for p in item["frame_paths"]:            
                liver_img = cv2.imread(p["liver_path"])
                lesion_img = cv2.imread(p["lesion_path"])
            ...
            lesion_ratios.append(wt)
            imgs = torch.stack(imgs,dim=0)
            convlstm_images.append(imgs.type(torch.FloatTensor))
            features = torch.from_numpy(np.array(features)).float()

        label = torch.from_numpy(np.array(label)).long()
        convlstm_images = torch.stack(convlstm_images, dim = 0)

        return {"lm_frames": convlstm_images, "label": label, "image_num":image_num,"features":features}
